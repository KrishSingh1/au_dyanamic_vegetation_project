...
site_info = pd.read_csv("../Sturt_Plains_DEA_FC/Input/Sturt_Plains.csv") # get df for sites info 
print(site_info)
...
if debug == 0 or debug == 1 or debug == 2:
    ...
    ###### Create Query #######
    ## Load in query file (sites coords of interest)
    site_info = pd.read_csv("Input/Sturt_Plains.csv") # get df for sites info, I changed it from sites_info_query to subquery 
    ...
    end_date = datetime.date(2024, 1, 11) # the lower bound of the end date increment 
    ...
    extent_2 = (((2*extent)**(1/2))/2)/111000 # find length (in degrees) of the half diagonal of the extent (based on 1 degree = 111,000 metres)
    ...
    for RI in range(len(site_info)):
      ...
      y1, y2 = site_info["Lat"][RI] - extent_2, site_info["Lat"][RI] + extent_2 
      x1, x2 = site_info["Long"][RI] - extent_2,  site_info["Long"][RI] + extent_2
      ...
      for time in times_query:
        ...
        write_to_log(f"Start Query ({site_info['Location'][RI]}, Index = {RI})\n{query}") # write_to_log query
        ...
        if fc: # check if the query was found, (i.e., the retrieved data is not null)
            try:
              ...
              if wo: # check if the query was found, (i.e., the retrieved data is not null)
                ...
                write_to_log(f"Query (sucessful!) ({site_info['Location'][RI]}, Index = {RI})\n{query}") # write_to_log query
            except:
              write_to_log(f"Query (unsucessful) ({site_info['Location'][RI]}, Index = {RI})\n{query}") # write_to_log query

      # Export site data as a csv file
      file_name = f"{site_info['Location'][RI]}.csv" # Simply location_name.csv
      file_path = os.path.join(output_path,file_name)
      sites_results_df.reset_index(drop= True).to_csv(file_path, index = False) # Reset the index, then export as csv

      write_to_log(f"End Export ({site_info['Location'][RI]})") # write_to_log query
