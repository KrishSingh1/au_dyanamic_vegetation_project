...
site_merged = datasets[sites_list[0]].copy().dropna(subset = FEATURES) # drop na based on chosen features, needed for random forest 
scores = []
...
train_r = site_merged.iloc[site_merged.index <= time_split]
test_r = site_merged.iloc[site_merged.index > time_split]
...
fold = 0
for train_idx, val_idx in tss.split(train_r): # split train into k-folds 
    train_f = train_r.iloc[train_idx]
    val_f = train_r.iloc[val_idx]
...
default_RF_CV = pd.DataFrame(cross_validate(reg, X = train_r[FEATURES],
                     y = train_r[TARGET], cv=tss, 
...
for site in sites_list:
    site_merged = datasets[site].copy().dropna(subset = FEATURES)
    train = site_merged.iloc[site_merged.index <= time_split]
    
    reg.n_estimators += 10
    reg.fit(train[FEATURES], train[TARGET])
        
    scores_train.append(mean_squared_error(reg.predict(train_r[FEATURES]), train_r[TARGET]))
    scores_test.append(mean_squared_error(reg.predict(test_r[FEATURES]), test_r[TARGET]))
...
site_merged = datasets['NSANAN0002'].copy().dropna(subset = FEATURES)
