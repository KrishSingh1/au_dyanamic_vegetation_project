...
def plotPredictions(actual, prediction, TARGET, msg = '', split = '', fire_split = '', save_figure_path = ''):
    ...
    for i,v in enumerate(TARGET):
        ...
        ax[i].axvspan(split[0],split[1], alpha=0.3)
        ...
        if save_figure_path:
            fig.savefig(f'{save_figure_path}/{msg}.png')
...
# This is if we want to use a defined bioregion of interest
bioclimatic_region_classifications = pd.read_csv('..\DATASETS\Australian Bioclimatic Regions\AusPlots_BioclimaticRegion_Classified.csv', index_col = 0)
classifications = pd.read_csv('..\DATASETS\Australian Bioclimatic Regions\Classifications_Reference.csv', index_col = 0) # Our classification code 
bioregion_of_interest = 'Warm Temperate' # The bioregion classification of interest 
code = classifications[classifications['Classification'] == bioregion_of_interest]['Code'].values[0] # extract the code assossicated with the bioregion of interest 
bioregion_sites = list(bioclimatic_region_classifications[bioclimatic_region_classifications['Group_Code'] == code].index) # get all sites that belong to that class

#%% Set the list of AusPlots sites to model
...
bioregion_of_interest_folder = '_'.join(bioregion_of_interest.split(' '))
save_figure_path = f'..\RESULTS\Modelled_Results\{bioregion_of_interest_folder}'
save_figure_path = ''
...
for site in sites_list:
    ...
    plotPredictions(site_data, df, TARGET, msg = f'{site}_MSE_Scores_train_{train_score:.2f}_test_{test_score:.2f}',
...
n_repeats = 50
perm_importance = permutation_importance(reg, test_merged[FEATURES], np.array(test_merged[TARGET])[:,1],
...
perm_importance_df_2.plot.barh(figsize = (15, 10), ax = ax[1], label = 'Mean Permutation Importance (MSE)')
fig.tight_layout()


#fig.savefig(f'{save_figure_path}/variable_importance_{bioregion_of_interest_folder}')
...
# plots = os.listdir(save_figure_path)
# plots = [p for p in plots if p.endswith('.png')]

# for p in plots:
#     doc.add_heading(f'{p}', level = 1)
#     doc.add_picture(f'{save_figure_path}/{p}', width=Inches(5))
#     doc.add_page_break()
    
# doc.save(f'{save_figure_path}/Word_Document/Visualisation_{bioregion_of_interest_folder}.docx')


# for i in range(5):
#     doc.add_heading(f'Plot {i}', level=1)
#     doc.add_picture(f'plot_{i}.png', width=Inches(5))  # Adjust the width as needed
#     doc.add_page_break()  # Add a page break after each plot
