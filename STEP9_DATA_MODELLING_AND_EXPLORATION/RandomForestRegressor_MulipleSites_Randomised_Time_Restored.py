...
def plotPredictions(actual, prediction, TARGET, msg = '', split = '', fire_split = ''):
    ...
    for i,v in enumerate(TARGET):
        actual[v].plot(ax=ax[i], color = 'blue', alpha = 0.4, linestyle='dashed', label = f'Observed {v.split("_")[0].upper()}')
        prediction[f'prediction_{v}'].plot(ax=ax[i], color = 'orange', ylim = (0,100), label = f'Modelled {v.split("_")[0].upper()}')
        ax[i].legend()
        ax[i].grid(True)
        if split:
            ax[i].axvspan(split[0], split[1], alpha=0.05)
            for s in split:
